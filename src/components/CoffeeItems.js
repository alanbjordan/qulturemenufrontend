...
const CoffeeItems = ({ goToMainMenu, cartItems, setCartItems }) => {
  ...
  useEffect(() => {
    const getMenuItems = async () => {
      ...
      const coffeeItems = items.filter(item => item.category_id === '6be427ba-99fc-4fba-8520-68594ce4b747'); // Replace with actual Coffee category ID
    ...
    const loadData = async () => {
      ...
      setTimeout(() => {
        setLoading(false); // Spinner will spin for at least 2 seconds
  ...
  return (
    <div className='main'>
      <header className='Logo-header text-center p-3'>
        <img src={coffeeHeader} alt='Qulture Image' className='img-fluid mb-3'/>
      </header>
