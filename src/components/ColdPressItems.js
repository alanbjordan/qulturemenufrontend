...
const ColdPressItems = ({ goToMainMenu, cartItems, setCartItems }) => {
  const [allColdPressItems, setAllColdPressItems] = useState([]);
  ...
  useEffect(() => {
    const getMenuItems = async () => {
      ...
      const additionalCategoryItems = items.filter(item => item.category_id === 'some-other-category-id'); // Replace with appropriate category ID

      const allItems = [...coldPressItems, ...additionalCategoryItems].sort((a, b) => {
